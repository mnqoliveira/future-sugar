...
if __name__ == '__main__':
    ...
    # dat = proc_apsim(BASE + SUBDIRS[0], 'J0QX1111_1980.out', AGG_FOO, DATA_DIC, True)
    # dat.to_csv('header_apsim.csv')
    for subdir in SUBDIRS:
        targets = [x for x in  os.listdir(BASE + subdir)if x.endswith('.out')]
        try:
            done = pd.read_csv('apsimProc.csv', usecols=[1], names=['file'])
        ...
        for targ in targets:
            ...
            try:
                ...
                with open('apsimProc.csv', 'a') as f:
                    dat.to_csv(f, header=False)
            except pd.errors.ParserError:
                dat = proc_apsim(BASE + subdir, targ, AGG_FOO, DATA_DIC, False)
                with open('apsimProc.csv', 'a') as f:
                    dat.to_csv(f, header=False)
                with open('bad_lines.csv', 'a') as f:
                    f.write(targ + '\n')
            except UnicodeDecodeError:
                with open('corrupted.csv', 'a') as f:
                    f.write(targ + '\n')
