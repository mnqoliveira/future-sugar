...
if __name__ == '__main__':
    ...
    try:
        done = pd.read_csv('apsimProc_resi.csv', usecols=[1], names=['file'])
    ...
    for targ in targets:
        ...
        try:
            ...
            with open('apsimProc_resi.csv', 'a') as f:
                dat.to_csv(f, header=False)
        except pd.errors.ParserError:
            dat = proc_apsim(BASE + subdir, targ, AGG_FOO, DATA_DIC, False)
            with open('apsimProc_resi.csv', 'a') as f:
                dat.to_csv(f, header=False)
            with open('bad_lines_resi.csv', 'a') as f:
                f.write(targ + '\n')
        except UnicodeDecodeError:
            with open('corrupted_resi.csv', 'a') as f:
                f.write(targ + '\n')
